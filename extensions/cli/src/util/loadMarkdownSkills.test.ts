...
describe("loadMarkdownSkills", () => {
  ...
  it("loads a valid skill with files from .tezz/skills", async () => {
    const skillDir = path.join(tmpDir, ".tezz", "skills", "my-skill");
...
`,
  ...
  it("returns error for invalid frontmatter", async () => {
    const skillDir = path.join(tmpDir, ".tezz", "skills", "bad-skill");
    fs.mkdirSync(skillDir, { recursive: true });
    fs.writeFileSync(
      path.join(skillDir, "SKILL.md"),
      `---
...
`,
  ...
  it("loads multiple skills from different directories", async () => {
    const skill1Dir = path.join(tmpDir, ".tezz", "skills", "skill-1");
    const skill2Dir = path.join(tmpDir, ".tezz", "skills", "skill-2");
