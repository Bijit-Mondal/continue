...
models:
  - model: gpt-4
    ...
    expect(output).toContain('tezz -p "please review my current git diff"');
    expect(output).toContain('echo "hello" | tezz -p');
    expect(output).toContain('tezz -p "analyze the code in src/"');
  });
