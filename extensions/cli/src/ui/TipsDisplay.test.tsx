...
import { TipsDisplay, shouldShowTip, TEZZ_CLI_TIPS } from "./TipsDisplay.js";
...
describe("TipsDisplay", () => {
  ...
  it("renders a random tip from the tips array", () => {
    ...
    const expectedTipIndex = Math.floor(0.5 * TEZZ_CLI_TIPS.length);
    const expectedTip = TEZZ_CLI_TIPS[expectedTipIndex];
  ...
  it("always renders the tip structure correctly", () => {
    ...
    const containsAtLeastOneTip = TEZZ_CLI_TIPS.some(
...
describe("TEZZ_CLI_TIPS", () => {
  it("contains at least 6 tips", () => {
    expect(TEZZ_CLI_TIPS.length).toBeGreaterThanOrEqual(6);
  });

  it("all tips are non-empty strings", () => {
    TEZZ_CLI_TIPS.forEach((tip, index) => {
  ...
  it("tips are helpful and informative", () => {
    ...
    const allTipsText = TEZZ_CLI_TIPS.join(" ").toLowerCase();
