...
describe("resolveReviews local discovery", () => {
  ...
  it("discovers files from .tezz/agents/", async () => {
    vi.mocked(fs.existsSync).mockImplementation((p) => {
      return p === path.join("/test/repo", ".tezz", "agents");
    });
    vi.mocked(fs.readdirSync).mockImplementation(((p: fs.PathLike) => {
      if (p === path.join("/test/repo", ".tezz", "agents")) {
  ...
  it("discovers files from .tezz/checks/", async () => {
    vi.mocked(fs.existsSync).mockImplementation((p) => {
      return p === path.join("/test/repo", ".tezz", "checks");
    });
    vi.mocked(fs.readdirSync).mockImplementation(((p: fs.PathLike) => {
      if (p === path.join("/test/repo", ".tezz", "checks")) {
        return ["anti-slop.md"];
      }
      return [];
    }) as typeof fs.readdirSync);
