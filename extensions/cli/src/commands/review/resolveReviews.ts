 ...
 * 3. Local .tezz/agents/*.md and .tezz/checks/*.md (fallback)
 */
export async function resolveReviews(
  ...
  // Source 3: Local .tezz/agents/*.md and .tezz/checks/*.md
...
/**
 * Resolve reviews from local .tezz/agents/*.md and .tezz/checks/*.md files.
...
function resolveFromLocal(): ResolvedReview[] {
  ...
  const dirs = [
    path.join(cwd, ".tezz", "agents"),
    path.join(cwd, ".tezz", "checks"),
  ];
