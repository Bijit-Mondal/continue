...
function renderTextReport(
  ...
  if (options.checksFromHub) {
    lines.push(
      isTTY
        ? chalk.dim("These reviews also run on your PRs via Tezz CI.")
        : "These reviews also run on your PRs via Tezz CI.",
    );
  } else {
    lines.push(
      isTTY
        ? chalk.dim(
            "Tip: Run these automatically on every PR → https://tezz.dev",
          )
        : "Tip: Run these automatically on every PR → https://tezz.dev",
    );
  }
