  ...
  if (reviews.length === 0) {
    ...
    console.log(
      chalk.yellow("\nNo reviews found. To add reviews:\n") +
        chalk.dim(
          "  1. Create .tezz/agents/my-review.md with agent instructions\n",
        ) +
        chalk.dim(
          "  2. Or specify an agent: tezz review --review-agents org/agent-name\n",
        ) +
        chalk.dim("  3. Or configure reviews on https://tezz.dev\n"),
    );
    process.exit(0);
  }
